...
CREATE TABLE job (
  ...
  requireNumber INT(11) DEFAULT NULL COMMENT '需要人数',
...
--
-- Definition for table talentLanguage
--
DROP TABLE IF EXISTS talentLanguage;
...
--
-- Definition for procedure getDealHistoryF_hunter_enterprise
--
DROP PROCEDURE IF EXISTS getDealHistoryF_hunter_enterprise$$
CREATE DEFINER = 'root'@'localhost'
PROCEDURE getDealHistoryF_hunter_enterprise(IN iTalent     INT,
                                                IN iHunter     INT,
                                                IN iEnterprise INT,
                                                IN iOffset     INT,
                                                IN iLimit      INT)
  COMMENT '取得特定人才和猎头和企业交易历史状态
in:
  iTalent
  iHunter
  iEnterprise
...
BEGIN
  ...
  JOIN talent f
  ON b.talent = f.id
  LEFT JOIN enterpriseUser g
  ON a.role = 'enterpriseUser' AND a.roleId = g.id AND g.active = 1
  LEFT JOIN hunter h
  ON a.role = 'hunter' AND a.roleId = h.id AND h.active = 1
  JOIN dealStatus i
  ON a.status = i.id
  WHERE
    a.active = 1
...
--
-- Definition for procedure getDealNumF_hunter
--
DROP PROCEDURE IF EXISTS getDealNumF_hunter$$
CREATE DEFINER = 'root'@'localhost'
PROCEDURE getDealNumF_hunter(IN iHunter INT)
  COMMENT '取得特定猎头交易数
in:
  iHunter 猎头id
out:
...
BEGIN
  ...
  WHERE
    ...
    c.status >= 7
    AND
    /*取当前状态即在dealHistory中相同deal中id最大的纪录*/
    NOT EXISTS (SELECT
                  1
                FROM
                  dealHistory aa
...
--
-- Definition for procedure getFitIndustryF_talent
--
DROP PROCEDURE IF EXISTS getFitIndustryF_talent$$
CREATE DEFINER = 'root'@'localhost'
PROCEDURE getFitIndustryF_talent(IN iTalent INT)
  COMMENT '取得特定人才的适合行业
in:
  iTalent
out:
...
CREATE DEFINER = 'root'@'localhost'
PROCEDURE getFitTalentF_job(IN iJob    INT,
                                IN iIsNum  BOOLEAN,
                                IN iOffset INT,
                                IN iLimit  INT)
...
BEGIN
  ...
  , education
  INTO
    vid, vCity, vBusinessArea, vSex, vAgeFrom, vAgeTo, vHeightFrom, vHeightTo, vEducation
  FROM
    job
  WHERE
    id = iJob;
  ...
  THEN
    ...
    ELSE
      SET @sql1 = 'SELECT a.id FROM talent a WHERE';
    END IF;
    ...
    ELSE
      SET @sql = concat(@sql1, @sql2, ' LIMIT ', vOffset, ', ', vLimit);
    END IF;
  ...
  ELSE
    ...
    THEN
      SELECT 0 num;
    ELSE
      SELECT a.id FROM talent a WHERE 0;
    END IF;
  END IF;
END
$$
...
CREATE DEFINER = 'root'@'localhost'
PROCEDURE getFitTalentSearchF_job(IN iJob          INT,
                                      IN iPersonName   VARCHAR(255),
                                      IN iHunter       INT,
                                      IN iSex          INT,
                                      IN iHeightFrom   INT,
                                      IN iHeightTo     INT,
...
BEGIN
  ...
  THEN
    ...
    IF iPersonName IS NOT NULL
    THEN
      SET @sql1 = concat(@sql1, ' a.personName LIKE "%', iPersonName, '%" AND');
    END IF;
...
--
-- Definition for procedure getHunterToDoF_talent_enterprise
--
DROP PROCEDURE IF EXISTS getHunterToDoF_talent_enterprise$$
CREATE DEFINER = 'root'@'localhost'
PROCEDURE getHunterToDoF_talent_enterprise(IN iTalent     INT,
                                               IN iEnterprise INT,
                                               IN iOffset     INT,
                                               IN iLimit      INT)
  COMMENT '取得特定人才和企业，猎头待办事项
in:
  iTalent
  iEnterprise
  iOffset
...
--
-- Definition for procedure getInfoF_hunter_enterprise_dealStatus
--
DROP PROCEDURE IF EXISTS getInfoF_hunter_enterprise_dealStatus$$
CREATE DEFINER = 'root'@'localhost'
PROCEDURE getInfoF_hunter_enterprise_dealStatus(IN iHunter     INT,
                                                    IN iEnterprise INT,
                                                    IN iStatus     INT)
  COMMENT '取得特定猎头对特定企业的特定交易状态的人才信息
in:
  iHunter
  iEnterprise
  iStatus
...
BEGIN
  ...
  , d.id education
  , e.name appearanceName
  , e.id appearance
  , f.name expressionName
  , f.id expression
  , a.hunterNote
  , a.release
  ...
  ON a.education = d.id
  AND d.active = 1
  LEFT JOIN commonLevel e
  ON a.appearance = e.id
  AND e.active = 1
  LEFT JOIN commonLevel f
...
BEGIN
  SELECT
    (sum(a.accurate) + sum(a.communication) + sum(a.quality)) / (count(*) * 3) point
  FROM
    deal a
  JOIN talent b
  ON a.talent = b.id
  ...
  WHERE
    ...
    c.status >= 7
    AND
    a.accurate IS NOT NULL
    AND
    a.communication IS NOT NULL
    AND
    a.quality IS NOT NULL
    AND
    /*取当前状态即在dealHistory中相同deal中id最大的纪录*/
    NOT EXISTS (SELECT
                  1
...
--
-- Definition for procedure getRemarkFromEnterpriseF_hunter
--
DROP PROCEDURE IF EXISTS getRemarkFromEnterpriseF_hunter$$
CREATE DEFINER = 'root'@'localhost'
PROCEDURE getRemarkFromEnterpriseF_hunter(IN iHunter INT)
  COMMENT '取得特定猎头得到的评语'
BEGIN
  SELECT
    c.name
...
--
-- Definition for procedure getTalentSearchF_hunter
--
DROP PROCEDURE IF EXISTS getTalentSearchF_hunter$$
CREATE DEFINER = 'root'@'localhost'
PROCEDURE getTalentSearchF_hunter(IN iPersonName   VARCHAR(255),
                                      IN iHunter       INT,
                                      IN iSex          INT,
                                      IN iHeightFrom   INT,
                                      IN iHeightTo     INT,
...
BEGIN
  ...
  IF vIsNum
  THEN
    SET @sql = 'SELECT count(*) num FROM talent a WHERE';
  ELSE
    SET @sql = 'SELECT a.id FROM talent a WHERE';
  END IF;

  SET @sql = concat(@sql, ' a.active = 1');

  IF iPersonName IS NOT NULL
  THEN
    SET @sql = concat(@sql, ' AND a.personName LIKE "%', iPersonName, '%"');
  END IF;
  IF iHunter IS NOT NULL
  THEN
    SET @sql = concat(@sql, ' AND a.hunter =', iHunter);
...
--
-- Definition for procedure getUnfitIndustryF_talent
--
DROP PROCEDURE IF EXISTS getUnfitIndustryF_talent$$
CREATE DEFINER = 'root'@'localhost'
PROCEDURE getUnfitIndustryF_talent(IN iTalent INT)
  COMMENT '取得特定人才的不适合行业
in:
  iTalent
out:
  array'
...
CREATE DEFINER = 'root'@'localhost'
PROCEDURE test(IN iHunter INT)
  COMMENT '取得特定猎头推荐成功数\\\\r
in:\\\\r
  iHunter 猎头id\\\\r
out:\\\\r
  int'
SELECT   count(*), b.hunter FROM   deal a JOIN talent b ON a.talent = b.id JOIN dealHistory aa ON a.id = aa.deal WHERE   a.active = 1   AND   b.active = 1   AND   b.hunter = iHunter   AND   aa.status >= 7   AND   NOT EXISTS (SELECT                 *               FROM                 dealHistory bb               WHERE                 bb.deal = aa.deal                 AND                 aa.id < bb.id)
$$

--
-- Definition for procedure test2
--
...
BEGIN
  ...
  DO
    ...
    SELECT
      NULL /*concat('photoURL', i)*/
    INTO
      vPhotoURL;

    SELECT
      NULL /*concat('videoURL', i)*/
    INTO
      vVideoURL;
    ...
    WHERE
      id =
      CASE
      WHEN mod(i, vLanguageNum) = 0 THEN
        vLanguageNum
      ELSE
        mod(i, vLanguageNum)
      END
    ...
    IF mod(i, vLanguageNum * 2) > 0 AND mod(i, vLanguageNum * 2) <= vLanguageNum
    THEN
      INSERT INTO `talentLanguage` (`talent`, `language`, `commonLevel`, `created`) VALUES (vTalent, vLanguage, vLanguageLevel, NULL);
    END IF;
    SET i = i + 1;
  END WHILE;
END
...
--
-- Definition for function createBonusF_job
--
DROP FUNCTION IF EXISTS createBonusF_job$$
CREATE DEFINER = 'root'@'localhost'
FUNCTION createBonusF_job(iJob INT, iBonus INT)
  RETURNS int(11)
  COMMENT '为特定工作创建奖金类型
in:
  iJob
...
BEGIN
  ...
  INSERT INTO `jobBonus` (`job`, `bonus`, `updated`, `created`) VALUES (iJob, iBonus, NULL, NULL);
  IF ROW_COUNT() = 1
  THEN
    SET vResult = 1;
  ELSE
...
CREATE DEFINER = 'root'@'localhost'
FUNCTION createDeal(iTalent INT, iJob INT, iEnterpriseUser INT, iStatus INT, iNote TEXT)
  RETURNS int(11)
  COMMENT '创建新交易
in:
  iTalent
...
BEGIN
  ...
  THEN
    ...
    INSERT INTO `dealHistory` (`deal`, `status`, `note`, `role`, `roleId`, `updated`, `created`) VALUES (LAST_INSERT_ID(), iStatus, iNote, 'enterpriseUser', iEnterpriseUser, NULL, NULL);
    IF ROW_COUNT() = 1
    THEN
      SET vResult = 1;
    ELSE
...
--
-- Definition for function createLanguageF_job
--
DROP FUNCTION IF EXISTS createLanguageF_job$$
CREATE DEFINER = 'root'@'localhost'
FUNCTION createLanguageF_job(iJob INT, iLanguage INT, iCommonLevel INT)
  RETURNS int(11)
  COMMENT '为特定工作创建语言要求
in:
  iJob
...
BEGIN
  ...
  SET sql_mode = 'STRICT_ALL_TABLES';
  INSERT INTO `jobLanguage` (`job`, `language`, `commonLevel`, `updated`, `created`) VALUES (iJob, iLanguage, iCommonLevel, NULL, NULL);
  IF ROW_COUNT() = 1
  THEN
    SET vResult = 1;
  ELSE
...
--
-- Definition for function createStatusF_talent_job
--
DROP FUNCTION IF EXISTS createStatusF_talent_job$$
CREATE DEFINER = 'root'@'localhost'
FUNCTION createStatusF_talent_job(iTalent INT, iJob INT, iStatus INT, iNote TEXT, iRole VARCHAR(255), iRoleId INT)
  RETURNS tinyint(4)
  COMMENT '为特定人才,工作所属交易设置状态
in:
  iTalent
  iJob
  iStatus
...
BEGIN
  ...
  THEN
    ...
    END IF;
  END IF;
  INSERT INTO `dealHistory` (`deal`, `status`, `note`, `role`, `roleId`, `updated`, `created`) VALUES (vDeal, iStatus, iNote, iRole, iRoleID, NULL, NULL);
  IF ROW_COUNT() = 1
  THEN
    SET vResult = 1;
  ELSE
...
--
-- Definition for function createWelfareF_job
--
DROP FUNCTION IF EXISTS createWelfareF_job$$
CREATE DEFINER = 'root'@'localhost'
FUNCTION createWelfareF_job(iJob INT, iWelfare INT)
  RETURNS int(11)
  COMMENT '为特定工作创建福利类型
in:
  iJob
...
BEGIN
  ...
  SET sql_mode = 'STRICT_ALL_TABLES';
  INSERT INTO `jobWelfare` (`job`, `welfare`, `updated`, `created`) VALUES (iJob, iWelfare, NULL, NULL);
  IF ROW_COUNT() = 1
  THEN
    SET vResult = 1;
  ELSE
...
--
-- Definition for function updateDealEnterpriseNote
--
DROP FUNCTION IF EXISTS updateDealEnterpriseNote$$
CREATE DEFINER = 'root'@'localhost'
FUNCTION updateDealEnterpriseNote(iTalent INT, iJob INT, iEnterpriseNote TEXT)
  RETURNS int(11)
  COMMENT '企业用户为交易填写备注
in:
  iTalent
...
BEGIN
  ...
  UPDATE `deal`
  SET
    `enterpriseNote` = iEnterpriseNote, `updated` = NULL
  WHERE
    id = vDeal;
...
CREATE DEFINER = 'root'@'localhost'
FUNCTION updateHunter(iHunter INT, iPassword VARCHAR(255), iFixphone1 VARCHAR(255), iFixphone2 VARCHAR(255), iFixphone3 VARCHAR(255), iMobile VARCHAR(255), iPersonName VARCHAR(255), iIdNo VARCHAR(255), iAddress VARCHAR(255), iBankNo VARCHAR(255), iBankName VARCHAR(255))
  RETURNS int(11)
  COMMENT '更新猎头
in:
  iHunter
...
BEGIN
  ...
  UPDATE `hunter`
  SET
    `password` = iPassword, `fixphone1` = iFixphone1, `fixphone2` = iFixphone2, `fixphone3` = iFixphone3, `mobile` = iMobile, `personName` = iPersonName, `idNo` = iIdNo, `address` = iAddress, `bankNo` = iBankNo, `bankName` = iBankName, `updated` = NULL
  WHERE
    id = iHunter;
  IF ROW_COUNT() = 1
  THEN
...
--
-- Definition for function updateJob
--
DROP FUNCTION IF EXISTS updateJob$$
CREATE DEFINER = 'root'@'localhost'
FUNCTION updateJob(iJob INT, iTitle VARCHAR(255), iRequireNumber INT, iWorkType INT, iContractType INT, iWorkTime INT, iCity INT, iBusinessArea INT, iOnBoardDate DATE, iSalaryFrom INT, iSalaryTo INT, iCommissionDate INT, iSex INT, iAgeFrom INT, iAgeTo INT, iHeightFrom INT, iHeightTo INT, iEducation INT, iSpecialSkill TEXT, iDetail TEXT)
  RETURNS tinyint(1)
  COMMENT '更新职位
in:
  iJob
...
UPDATE `job`
SET
  `title` = iTitle, `requireNumber` = iRequireNumber, `workType` = iWorkType, `contractType` = iContractType, `workTime` = iWorkTime, `city` = iCity, `businessArea` = iBusinessArea, `onboardDate` = iOnBoardDate, `salaryFrom` = iSalaryFrom, `salaryTo` = iSalaryTo, `commissionDate` = iCommissionDate, `sex` = iSex, `ageFrom` = iAgeFrom, `ageTo` = iAgeTo, `heightFrom` = iHeightFrom, `heightTo` = iHeightTo, `education` = iEducation, `specialSkill` = iSpecialSkill, `detail` = iDetail, `updated` = NULL
WHERE
  id = iJob;
  IF ROW_COUNT() = 1
  THEN
